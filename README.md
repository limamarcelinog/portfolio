...
| [**Sales call auditing with an LLM**](projects/call-audit-llm/) | Python · Claude API · Pydantic | Rubric-based call scoring where the engineering is the guards, not the model call: behavioural anchors so the model classifies instead of judging, every score required to cite a quote that is checked against the transcript, the scorer's own run-to-run noise measured and published, and an aggregate that refuses to rank a rep on thin data or on a gap inside its error bars |
